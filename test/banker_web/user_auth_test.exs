defmodule BankerWeb.UserAuthTest do
  use BankerWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Banker.Accounts
  alias Banker.Accounts.Scope
  alias BankerWeb.UserAuth

  import Banker.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BankerWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      BankerWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        BankerWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
